'use client'

import { useMemo, useState } from 'react'
import { AdminFormField, adminInputClass } from '@/components/admin/AdminFormField'
import { AdminModal } from '@/components/admin/AdminModal'
import { AdminTable } from '@/components/admin/AdminTable'
import { AdminPageHeader, Toggle } from '@/components/admin/AdminUi'
import { useAdminCollection } from '@/components/admin/useAdminCollection'

interface FooterLink {
  id: string
  section: 'shop' | 'company'
  name: string
  href: string
  isActive: boolean
  sortOrder: number
}

const initialForm: Partial<FooterLink> = {
  section: 'shop',
  name: '',
  href: '/shop',
  isActive: true,
  sortOrder: 1,
}

export default function AdminFooterLinksPage() {
  const { data, loading, saving, save, remove } = useAdminCollection<FooterLink>('footer-links')
  const [open, setOpen] = useState(false)
  const [form, setForm] = useState<Partial<FooterLink>>(initialForm)

  const sorted = useMemo(
    () => [...data].sort((a, b) => Number(a.sortOrder || 999) - Number(b.sortOrder || 999)),
    [data],
  )

  return (
    <div>
      <AdminPageHeader
        title="Footer Links"
        onAdd={() => {
          setForm(initialForm)
          setOpen(true)
        }}
      />
      <AdminTable
        columns={[
          { key: 'section', label: 'Section' },
          { key: 'name', label: 'Name' },
          { key: 'href', label: 'URL' },
          { key: 'sortOrder', label: 'Sort' },
          { key: 'isActive', label: 'Status', render: (v) => (v ? 'Active' : 'Inactive') },
        ]}
        data={sorted}
        loading={loading}
        onEdit={(item) => {
          setForm(item)
          setOpen(true)
        }}
        onDelete={(id) => void remove(id)}
      />

      <AdminModal
        open={open}
        onClose={() => setOpen(false)}
        title={form.id ? 'Edit Footer Link' : 'Add Footer Link'}
        saving={saving}
        onSave={async () => {
          await save(form)
          setOpen(false)
        }}
      >
        <AdminFormField label="Section">
          <select className={adminInputClass} value={form.section ?? 'shop'} onChange={(e) => setForm((p) => ({ ...p, section: e.target.value as FooterLink['section'] }))}>
            <option value="shop">Shop</option>
            <option value="company">Company</option>
          </select>
        </AdminFormField>
        <AdminFormField label="Name" required>
          <input className={adminInputClass} value={form.name ?? ''} onChange={(e) => setForm((p) => ({ ...p, name: e.target.value }))} />
        </AdminFormField>
        <AdminFormField label="URL" required>
          <input className={adminInputClass} value={form.href ?? ''} onChange={(e) => setForm((p) => ({ ...p, href: e.target.value }))} />
        </AdminFormField>
        <AdminFormField label="Sort Order">
          <input type="number" className={adminInputClass} value={form.sortOrder ?? 1} onChange={(e) => setForm((p) => ({ ...p, sortOrder: Number(e.target.value || 1) }))} />
        </AdminFormField>
        <AdminFormField label="Active">
          <Toggle checked={Boolean(form.isActive)} onChange={(next) => setForm((p) => ({ ...p, isActive: next }))} />
        </AdminFormField>
      </AdminModal>
    </div>
  )
}
