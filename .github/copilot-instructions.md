...
# MyGift.pk Design System

## Brand Identity
- **Brand**: MyGift.pk - Luxury Pakistani gifting e-commerce
- **Target**: Pakistani diaspora + domestic customers
- **Theme**: Dark luxury with gold accents
- **Personality**: Premium, elegant, exclusive

## Color Palette
```css
--ink: #0f0608;              /* Deep black background */
--surface: #1a0c10;          /* Dark surface layer */
--surface-2: #2a1a20;        /* Slightly lighter surface */
--gold: #c9a84c;             /* Accent gold */
--gold-light: #d4b55e;       /* Lighter gold */
--cream: #f5f1ed;            /* Off-white text */
--muted: #a8938a;            /* Muted gray text */
--wine: #6b1b3d;             /* Secondary accent (wine/maroon) */
--rose: #d98a94;             /* Tertiary accent (rose) */
--border: rgba(201, 168, 76, 0.15);  /* 15% gold at opacity */
--border-hover: rgba(201, 168, 76, 0.3); /* 30% gold on hover */
```

## Typography
- **Display/Hero** (`--font-heading`): Cormorant Garamond (300, 400, 600)
  - H1: 40px mobile, 48px+ desktop, light weight
  - H2: 32px mobile, 40px+ desktop
  
- **Accent/Labels** (`--font-accent`): Cinzel (400, 600)
  - Used for: nav labels, section eyebrows, "UPPERCASE" text
  - Size: 10-12px, tracking 0.5em (wide letter spacing)
  
- **Body/UI** (`--font-sans`): DM Sans (300, 400, 500)
...
### Buttons
```
- NO rounded corners above 8px (use square or 4px)
- NO drop shadows
- Gold border on hover: border-color: var(--gold)
- Padding: px-6 py-2.5 (responsive)
- Text: All caps, tracking-widest, font-accent
- Variants: primary (filled), outline (border), ghost (text only)
```

### Cards & Panels
```
- Border: 1px solid rgba(201,168,76,0.15) [--border]
- Border hover: rgba(201,168,76,0.3)
- Background: var(--surface)
- No shadows, no rounded corners > 8px
- Padding: p-6 or p-8
- Spacing: gap-4 or gap-6 between cards
```

### Navigation
```
- Background: var(--ink)
- Links: font-accent, uppercase, tracking-[0.3em]
- Hover: text-gold with smooth transition
- Active: underline with gold
```
...
- Header: cream text, uppercase label
- Close: X icon (lucide-react), hover gold
- Animation: fadeIn or slideInRight (see Animate.css)
```

### Forms
```
- Input background: var(--surface)
- Input border: 1px var(--border)
- Input focus: border-gold, outline-none
- Label: font-accent, 10px, uppercase, tracking-widest
- Error: text-rose, border-rose
- Success: text-gold
```
...
- `p-6` / `gap-6` = 24px
- `p-8` / `gap-8` = 32px
- `py-8` / `py-12` / `py-16` for vertical sections
...
Mobile-first approach: write default styles, then override with breakpoints.

## Dark Theme Best Practices
- ✅ Use `--ink` and `--surface` for backgrounds
- ✅ Use `--cream` for primary text (high contrast)
- ✅ Use `--muted` for secondary text (lower contrast)
- ✅ Gold accents sparingly - max 1-2 per page
- ✅ Borders: use gold at low opacity (15%)
- ❌ Avoid white/light grays (too harsh)
- ❌ Avoid bright colors (clashes with dark theme)
- ❌ Avoid drop shadows (use borders instead)

## Animation Guidelines
- **Page entrance**: `fadeIn` or `fadeInUp`
- **Section scroll**: `fadeInUp` with threshold 0.15
- **Buttons**: `pulse` on hover, `bounceIn` on focus
- **Cards**: `zoomIn` or `fadeInUp` on scroll
- **Modals**: `zoomIn` entrance, `zoomOut` exit
- Speed: Use `animate__fast` (800ms) by default
- Delay: Stagger items with `animate__delay-*`
...
<section className="relative h-[500px] lg:h-[600px] overflow-hidden">
  ...
  <div className="absolute inset-0 bg-gradient-to-t from-[var(--ink)] to-transparent">
    <div className="mx-auto max-w-7xl h-full flex items-end pb-8 lg:pb-12 px-4 lg:px-8">
      <div>
        <span className="font-accent text-[10px] uppercase tracking-[0.5em] text-[var(--gold)]">
          Eyebrow
        </span>
        <h1 className="mt-3 font-heading text-4xl lg:text-5xl font-light text-[var(--cream)]">
          Hero Heading
        </h1>
      </div>
    </div>
...
<div className="text-center mb-12">
  ...
  <h2 className="mt-3 font-heading text-3xl lg:text-4xl font-light text-[var(--cream)]">
    Section Title
  </h2>
...
## Tailwind Customization (in tailwind.config)
Already configured in `tailwind.config.ts`:
- CSS variables for colors
- Font families from Google Fonts
- Custom animation names
- Dark mode (implicit - all styles are dark)

## Reserved Gold Usage
Only use `--gold` for:
- CTA buttons primary action
- Hover states on interactive elements
- Accents in hero/featured sections
- Section eyebrow text (labels)
- Borders on focus/hover

DO NOT use gold for:
- Large backgrounds
- Body text
- Multiple elements in rapid succession
...
**See `DESIGN.md` in project root for complete design system documentation.**

The DESIGN.md file includes:
- Full color palette with semantic meanings
- Typography scale and font usage
- Component patterns (buttons, cards, inputs, modals)
- Layout patterns (hero, grids, page structure)
- Animation guidelines
...
1. Always consult DESIGN.md first
2. Use exact color values (CSS variables)
3. Follow typography scale exactly
4. Use Tailwind classes, not inline styles
5. Test on mobile and desktop
6. Include proper alt text and ARIA labels
7. Respect prefers-reduced-motion
8. Animate only when it adds value
